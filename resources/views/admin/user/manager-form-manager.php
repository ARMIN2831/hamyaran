...
if (isset($post['submit'])) {
    ...
    if ($uri[2][0]=="add"){
        if ($newManager->add($form)) {
            echo '<p class="btn btn-success">ثبت مدیر با موفقیت انجام شد.</p>';
            redirect("manager/add/manager", 2);
        } else {
    ...
    if ($uri[2][0]=="edit"){
        if ($newManager->edit($form)) {
            echo '<p class="btn btn-success">ویرایش مشخصات مدیر با موفقیت انجام شد.</p>';
            redirect("manager/edit", 2);
        } else {
            echo '<p class="btn btn-warning">ویرایش مشخصات مدیر ناموفق بود.</p>';
        }
    }
...
<form method="post">
    <div class="card-header">
        <h4 class="card-title">مشخصات مدیر:</h4>
    </div>
    <div class="row">
        <div class="col-md-6">
        ...
        <div class="col-md-6">
            <fieldset class="form-group">
                ...
                <select class="custom-select" name="access[convene]" id="convene">
                    ...
                    foreach ($convenes as $convene) {
                        echo '<option ' . ($managerAccess['convene'] == $convene['ID'] ? "selected" : "") . ' value="' . $convene['ID'] . '">' . $convene['name'] . '</option>';
                    }
                    ?>
                </select>
            </fieldset>
        ...
        <div class="col-md-6">
            <fieldset class="form-group">
                ...
                <br>
                <label> <input type="checkbox" name="access[a]" value="1" <?=($managerAccess['a']?'checked':'')?> class="form-check-inline"> افزودن پشتیبان</label>
                <label> <input type="checkbox" name="access[c]" value="1" <?=($managerAccess['c']?'checked':'')?> class="form-check-inline"> ویرایش پشتیبان</label>
